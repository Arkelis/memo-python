...
Le document considère que le lecteur a un niveau \og prépa \fg{} en Python: la définition de fonctions, les boucles, les conditions et la manipulation des listes est supposée connue. La première partie concerne la programmation orientée objet et le modèle de données de Python. Les deux parties suivantes concernent des modules de la bibliothèque standard et des modules communautaires. Il y a de nombreux liens en bleu, ne pas hésiter à cliquer dessus!\bigskip
...
\begin{description}
	\item[{\sffamily Notation}] Toutes les fonctions sont notées avec des parenthèses : \mintinline{python3}{function()}.
...
\paragraph{Remarque} On utilise le préfixe \mintinline{text}{r} devant la chaîne de caractère pour éviter d'avoir à écrire \Verb[fontsize = \small]|\\| au lieu d'un unique \Verb[fontsize = \small]|\|.\bigskip
...
\section{\texttt{os} --- Diverses interfaces avec le système d'exploitation}\index{os}

...
\begin{description}
    \item[{\sffamily Notation}] Toutes les fonctions sont notées avec des parenthèses : \mintinline{python3}{fonction()}.	\item[{\bfseries\sffamily Notation}] Pour la documentation des méthodes:
        \begin{itemize}
...
\input{POO/Coroutines.tex}
\input{POO/Decorateurs}
\input{POO/Metaclasses.tex}
...
Cette \marginpar{\href{https://docs.python.org/fr/3/library/builtins.html}{\mintinline{python3}{builtins}}} section présente des fonctions et types accessibles dans l'interpréteur sans rien importer. Ce sont les \mintinline{python3}{builtins}.
...
\paragraph*{Remarque} Parfois, on est amemnés (même si ce n'est pas forcément recommandé) à nommer un variable d'après un objet natif. Pour retrouver ledit objet natif, on peut explicitement passer par le module \mintinline{python3}{builtins}.

\subsection{Interactions avec l'utilisateur}

\begin{description}
    \item[\mintinline{python3}{print(*objects, sep=' ', end="\n", file=sys.stdout, flush=False)}]~
    
    Affiche les \mintinline{python3}{objects} passés en paramètre sous forme de chaîne de caractères grâce au constructeur \mintinline{python3}{str()}. Si plusieurs objets sont passés en paramètre, ils sont séparés par \mintinline{python3}{sep}. La sortie se termine par \mintinline{python3}{end}. Ces deux derniers paramètres doivent être nommés, tous les arguments positionnels sont considérés objets à afficher. Si aucun objet n'est fourni, l'affiche que \mintinline{python3}{end}.
...
Python > OCaml > C > Java
    ...
    Afficher revient en fait à écrire dans un flux texte. Par défaut, il s'agit de la sortie standard (ce qui s'affiche sur la console). On peut changer la cible de la sortie avec le paramètre \mintinline{python3}{file} ; l'objet passé en paramètre doit avoir une méthode \mintinline{python3}{write(chaine)}. On peut forcer l'effacement du tampon de flux en mettant le paramètre \mintinline{python3}{flush} à \mintinline{python3}{True}.\medskip

    Pour afficher des progressions, on peut utiliser le retour chariot \mintinline{python3}{"\r"} comme chaîne finale :
...
Terminé !
    ...
    \item[\mintinline{python3}{input(prompt="")}]~ 
    
    Affiche \mintinline{python3}{prompt} et lit une ligne sur l'entrée standard qui est convertie en chaîne de caractères (en supprimant le retour à la ligne final).
...
\begin{description}
    \item[\mintinline{python3}{help([objet])}]~
    ...
    \item[\mintinline{python3}{breakpoint(*args, **kwargs)}]~
    
    Sans paramètre, place dans le débogueur. Cette fonction appelle en fait \mintinline{python3}{sys.breakpointhook()}. Il est possible de passer des paramètres à cette fonction, cf. \href{https://docs.python.org/fr/3/library/sys.html\#sys.breakpointhook}{la documentation}.

    \item[\mintinline{python3}{globals()}]~
    ...
    \item[\mintinline{python3}{locals()}]~
    
    Renvoie un dictionnaire contenant les variables locales. Si appelé au niveau d'un module, a le même comportement que \mintinline{python3}{globals()}.

    \item[\mintinline{python3}{vars([object])}]~
    
    Renvoie \mintinline{python3}{objet.__dict__} si \mintinline{python3}{objet} est renseigné, et \mintinline{python3}{locals()} sinon.

    \item[\mintinline{python3}{dir([object])}]~
    
    Renvoie la liste des variables l'espace de nom courant, et si \mintinline{python3}{object} est renseigné, tente de renvoyer la liste de ces attributs valides.
...
\begin{description}
    \item[\mintinline{python3}{class range}]~
    
    Les objets \mintinline{python3}{range} sont des objets séquentiels (donc itérables) dont les éléments sont
    calculés à la volée. Il s'agit d'un outil indispensable pour faire des boucles. Il y a deux manières de construire un objet \mintinline{python3}{range} :

    \begin{description}
        \item[\mintinline{python3}{range(fin)}] Construit un objet \mintinline{python3}{range} qui permet d'itérer sur tous les entiers de 0 à \mintinline{python3}{fin}.
            \begin{minted}{pycon}
>>> for i in range(5):
...     print(i)
...
...
4
            \end{minted}
        \item[\mintinline{python3}{range(debut, fin, pas=1)}] Construit un objet \mintinline{python3}{range} permettant d'itérer sur les entiers de \mintinline{python3}{debut} à \mintinline{python3}{fin} avec un pas de \mintinline{python3}{pas}. Lève une exception \mintinline{python3}{ValueError} si \mintinline{python3}{pas} vaut 0.
        \begin{minted}{pycon}
...
0
    ...
    Un objet \mintinline{python3}{range} n'est pas un itérateur.
...
TypeError: 'range' object is not an iterator
    ...
    Il s'agit d'un objet séquentiel itérable qui gère toutes les opérations communes sur de tels objets (indices, tranches, etc.). Les valeurs sont calculées à la volée grâce à la formule \mintinline{python3}{range_object[i] = debut + pas*i}, avec comme contraintes \mintinline{python3}{i >= 0} et \mintinline{python3}{range_object[i] < fin} si \mintinline{python3}{pas > 0} sinon \mintinline{python3}{range_object[i] > fin}.

    Attributs définis :
    \begin{description}
        \item[\mintinline{python3}{range.start}] Valeur du paramètre \mintinline{python3}{debut} passé au constructeur (0 par défaut).
        \item[\mintinline{python3}{range.stop}] Valeur du paramètre \mintinline{python3}{fin} passé au constructeur.
        \item[\mintinline{python3}{range.step}] Valeur du paramètre \mintinline{python3}{pas} passé au constructeur (1 par défaut).
    \end{description}


    \item[\mintinline{python3}{class zip}]~
    
    Les objets \mintinline{python3}{zip} permettent de construire des objets itérables agrégeant d'autres itérables.

    \begin{description}
        \item[\mintinline{python3}{zip(*iterables)}] Construit un objet \mintinline{python3}{zip} permettant d'itérer sur l'ensemble des itérables passés en paramètres à la fois. Le i-ème élément de cet itérateur est un tuple contenant les i-èmes éléments de chaque itérable. L'itérateur est épuisé lorsque dès qu'un des itérables en paramètre est épuisé.
    \end{description}
...
t, v, p, a
    ...
    \item[\mintinline{python3}{class enumerate}]~
    
    Les objets \mintinline{python3}{enumerate} permettent d'itérer et d'énumérer à la fois.

    \begin{description}
        \item[\mintinline{python3}{enumerate(iterable, start=0)}] Construit un objet \mintinline{python3}{enumerate} générant des tuples contenant un compte (démarrant à \mintinline{python3}{start}), et les objets résultant de l'itération sur l'itérable passé en paramètre.
    \end{description}
...
2 : c
    ...
    En quelque sorte, \mintinline{python3}{enumerate(iterable)} est l'équivalent de \mintinline{python3}{zip(range(len(iterable)), iterable)}.
...
\begin{description}
    \item[\mintinline{python3}{getattr(obj, name[, default])}]~
    
    Renvoie la valeur de l'attribut nommé \mintinline{python3}{name} de l'objet \mintinline{python3}{obj} : \mintinline{python3}{getattr(obj, "x")} renvoie \mintinline{python3}{obj.x}. Si \mintinline{python3}{name} n'est pas pris en charge par \mintinline{python3}{obj} (c'est-à-dire si \mintinline{python3}{obj.__getattribute__()} et \mintinline{python3}{obj.__getattr__()} ne parviennent pas à calculer \mintinline{python3}{name}), renvoie \mintinline{python3}{default} s'il est renseigné, ou lève une exception \mintinline{python3}{AttributeError} sinon.

    \item[\mintinline{python3}{hasattr(obj, name)}]~
    
    Permet de tester l'existence l'attribut nommé \mintinline{python3}{name} par \mintinline{python3}{obj} (\mintinline{python3}{hasattr(obj, "x")} teste l'existence de \mintinline{python3}{obj.x}) : renvoie \mintinline{python3}{False} si \mintinline{python3}{getattr(obj, name)} lève une exception \mintinline{python3}{AttributeError}, et \mintinline{python3}{True} sinon.


    \item[\mintinline{python3}{setattr(obj, name, value)}]~
    
    Assigne \mintinline{python3}{value} à l'attribut nommé \mintinline{python3}{name} de \mintinline{python3}{obj} : \mintinline{python3}{setattr(obj, "x", 1)} est équivalent à \mintinline{python3}{obj.x = 1}.
    
\end{description}

Ces fonctions sont équivalentes aux syntaxes classiques d'accès aux attributs et et de leur modificaion (i.e. \mintinline{python3}{obj.attr}, \mintinline{python3}{obj.attr = val}), elles ne permettent pas de d'éviter le mécanisme de descripteur ou les méthodes spéciales \mintinline{python3}{__getattribute__()}, \mintinline{python3}{__getattr__()} ou \mintinline{python3}{__setattr__()}.
...
\begin{description}
    \item[\mintinline{python3}{super(type_obj, obj)}]
    \item[\mintinline{python3}{isinstance(obj, classinfo)}]
    \item[\mintinline{python3}{issubclass(obj, classinfo)}]
\end{description}

\subsection{Le père et le créateur}

\begin{description}
    \item[\mintinline{python3}{class object}]~
    
    Classe parente de toutes les classes. Toutes les classes héritent implicitement d'\mintinline{python3}{object}.
    
    \begin{description}
        \item[\mintinline{python3}{object()}] Constructeur de la classe \mintinline{python3}{object}. Construit un objet vide auquel on ne peut assigner d'attribut et qui possède les méthodes communes à tous les objets, documentées ci-dessous.
        \item[\mintinline{python3}{object.__new__(classe)}] Crée un objet vide. \mintinline{python3}{object.__new__()} n'accepte qu'un type comme argument, sauf quand \mintinline{python3}{__init__()} est définie dans une classe, au quel cas \mintinline{python3}{object.__new__()} accepte tous les arguments de \mintinline{python3}{__init__()}. Si on veut surcharger \mintinline{python}{object.__new__()}, il faudra cependant penser à créer l'objet vide en appelant \mintinline{python3}{object.__new__()} avec la classe comme unique paramètre.
...
\end{minted}
        \item[\mintinline{python3}{object.__repr__(self)}] Renvoie une chaîne de caractère indiquant le type de l'objet ainsi que son adresse mémoire.
        \item[\mintinline{python3}{object.__str__(self)}] Renvoie \mintinline{python3}{object.__repr__(self)}. 
        \item[\mintinline{python3}{object.__dir__(self)}] Appelée quand \mintinline{python3}{dir()} est appelée sur un objet. Renvoie un itérable des attributs de l'objet.
        \item[\mintinline{python3}{object.__eq__(self, other)}] Appelée lors de l'opération \mintinline{python3}{objet == other}. Compare les valeurs de hachage et renvoie \mintinline{python3}{True} si elles sont égales.
        \item[\mintinline{python3}{object.__ne__(self, other)}] Appelée lors de l'opération \mintinline{python3}{objet != other}. Renvoie l'opposé de\break\mintinline{python3}{object.__eq__(self, other)}.
        \item[\mintinline{python3}{object.__ge__(self, other)}] Appelée lors de l'opération \mintinline{python3}{objet >= other}. Renvoie \mintinline{python3}{NotImplemented}.
        \item[\mintinline{python3}{object.__gt__(self, other)}] Appelée lors de l'opération \mintinline{python3}{objet > other}. Renvoie \mintinline{python3}{NotImplemented}.
        \item[\mintinline{python3}{object.__le__(self, other)}] Appelée lors de l'opération \mintinline{python3}{objet <= other}. Renvoie \mintinline{python3}{NotImplemented}.
        \item[\mintinline{python3}{object.__lt__(self, other)}] Appelée lors de l'opération \mintinline{python3}{objet < other}. Renvoie \mintinline{python3}{NotImplemented}.
        \item[\mintinline{python3}{object.__getattribute__(self, name)}] Appelée lorsqu'on accède à l'attribut \mintinline{python3}{name} avec la syntaxe \mintinline{python3}{self.name}. Cette méthode va chercher \mintinline{python3}{name} dans le dictionnaire de l'instance \mintinline{python3}{self.__dict__}. S'il est introuvable, elle
        va chercher successivement dans le dictionnaire du type de l'instance, puis dans toutes les classes parentes. Dans le cas où l'attribut est trouvé dans le dictionnaire d'une classe, s'il possède une méthode \mintinline{python3}{__get__()} (c'est-à-dire s'il s'agit d'un descripteur ou par extension d'une propriété), alors celle-ci sera appelée. 
        \item[\mintinline{python3}{object.__setattr__(self, name, value)}] Appelée lorsque l'on écrit \mintinline{python3}{self.name = value}. Le mécanisme de recherche d'attribut est le même que pour \mintinline{python3}{object.__getattribute__()}. Dans le cas d'un descripteur, c'est \mintinline{python3}{__set__()} qui est appelée.
        \item[\mintinline{python3}{object.__sizeof__(self)}] Renvoie la taille occupée en mémoire de \mintinline{python3}{self} en octets. 
    \end{description}

    \item[\mintinline{python3}{class type}]~
    
    Unique métaclasse (créateur de type) native.
    
    \begin{description}
        \item[\mintinline{python3}{type(objet)}] Passer un objet à \mintinline{python3}{type} renvoie la classe qui l'a instancié.
        \item[\mintinline{python3}{type(name, bases, attrs)}] Pour créer un type, on appelle le constructeur avec les paramètres suivant :
        \begin{enumerate}
            \item \mintinline{text}{name} : le nom du type à créer;
            \item \mintinline{text}{bases}: les classes parentes du type à créer (\mintinline{python3}{object} est implicite) dans un itérable;
            \item \mintinline{text}{attrs}: un dictionnaire contenant des attributs à affecter au type.
        \end{enumerate}
    \end{description}
\end{description}
...
\begin{description}
    \item [Spécification du caractère]~
          \begin{itemize}
              ...
              \item \og\mintinline{text}{\W}\fg{} désigne tout caractère non alpha-numérique.
              \item \og\mintinline{text}{\d}\fg{} équivaut à \mintinline{text}{[0-9]}.
              \item \og\mintinline{text}{\D}\fg{} désigne tout caractère non numérique.
              \item \og\mintinline{text}{\w}\fg{} équivaut à \mintinline{text}{[a-zA-Z0-9_]}.
              \item \og\mintinline{text}{\s}\fg{} désigne un espace.
          \end{itemize}
          \item [Place du motif dans la chaîne]~
          \begin{itemize}
            \item \og\mintinline{text}{^}\fg{} (se place au début) signifie que le début de la chaine doit correspondre au motif.
...
\paragraph{Remarque} On utilise le préfixe \mintinline{text}{r} devant la chaîne de caractère pour éviter d'avoir à écrire \mintinline{text}{\\} au lieu d'un unique \mintinline{text}{\}.\medskip
...
\end{enumerate}
et renvoie la chaine de caractères modifiée. Lorsque l'on veut appeler un groupe de caractères nommé avec \mintinline{text}{(?P<nom>)}, on y fait référence dans la chaine de remplacement par \mintinline{text}{\g<nom>}.
...
Puis on se place dans le dossier où l'on veut placer les environnements virtuels, par exemple sous Linux dans \mintinline{text}{/home/votre_nom/python_env/}, et on crée notre environnement!
...
Les modèles structurent la base de données et contiennent des métadonnées. Prenons un exemple musical et créons un modèle \mintinline{python3}{Artist}, un modèle \mintinline{python3}{Album} et un modèle \mintinline{python3}{Song}. On les implémente en tant que classes dans le fichier \mintinline{python3}{models.py}:
\begin{minted}{python3}
...
\begin{enumerate}
    ...
    \item \mintinline{python3}{artiste/1/} il cherche la ligne correspondante dans le fichier \mintinline{python3}{urls.py}. Il trouve alors la ligne \mintinline{text}{artiste/<artiste_id>}, il appelle donc la vue \mintinline{text}{artist(request=<HttpRequest object>, artist_id=1)}.
\end{enumerate}
...
Ainsi, si vous allez sur \mintinline{python3}{localhost:8000/artist/}, la liste de vos artistes s'affichera, ou bien \og Aucun artiste. \fg{} sinon.
...
La balise de gabarit \mintinline{text}|{% load static %}| génère l'URL absolue des fichiers statiques. Si on se rend à la page des artistes, tout est vert !
...
Comme \marginpar{\href{https://docs.djangoproject.com/fr/2.2/ref/models/}{\mintinline{python3}{django.db.models}}} indiqué dans le didacticiel:
\begin{enumerate}
    ...
    \item Cette application doit être mentionnée dans la liste \mintinline{python3}{INSTALLED_APPS} du fichier \mintinline{python3}{settings.py}
...
Un champ \marginpar{\href{https://docs.djangoproject.com/fr/2.2/ref/models/fields/\#field-types}{\mintinline{python3}{models.Field}}} de modèle doit être une instance de la classe \mintinline{python3}{Field} (où l'une de ses dérivées). Le choix du type de champ détermine le genre de donnée à stocker (par exemple des nombres ou du texte), le composants HTML qui sera utilisé dans le formulaire utilisé pour renseigner ce champs dans l'administration, et enfin les exigences minimales de validation de ce champ. Se référer aux liens dans la marge pour une documentation complète. Quelques types de champs génériques:
\begin{description}
    \item[\mintinline{python3}{class CharField(max_length=None, **options)}]~

          Un champ pour une chaîne de caractère (courte ou longue). Le paramètre \mintinline{python3}{max_length} règle la taille maximale de ce champ. Il en existe de plus précis pour les mails ou les URL, cf. la doc.

    \item[\mintinline{python3}{class DateField(auto_now=False, auto_now_add=False, **options)}]~

          Une date, représentée par la classe Python \hyperref[datetime]{\mintinline{python3}{datetime.date}}. Le paramètre \mintinline{python3}{auto_now} permet d'assigner automatiquement la date du jour à chaque enregistrement de l'objet, tandis que \mintinline{python3}{auto_now_add} enregistre la date du jour à la création de l'objet.
...
On \marginpar{\href{https://docs.djangoproject.com/fr/2.2/ref/models/fields/\#module-django.db.models.fields.related}{\mintinline{python3}{models.ForeignKey}}} peut aussi renseigner les relations entre les modèles (donc entre les tables de la base de données).
\begin{description}
    \item[\mintinline{python3}{class ForeignKey(to, on_delete, **options)}]~

          Une relation plusieurs-à-un, (cf. le didacticiel, exemple des chansons qui ont l'artiste en \mintinline{python3}{ForeignKey}). Cette classe exige la classe à laquelle le modèle et relié, et l'option \mintinline{python3}{on_delete}: \mintinline{python3}{models.CASCADE} si l'on veut que lorsque l'on supprime la \mintinline{python3}{ForeignKey}, que tous les objets associés du modèle concerné soient supprimés, ou bien \mintinline{python3}{SET_NULL} si l'on veut que les objets aient la valeur \mintinline{python3}{null} à la place de la \mintinline{python3}{ForeignKey} supprimée (dans ce cas il faut aussi renseigner \mintinline{python3}{null=True}). Il y a d'autres possibilités (cf. la doc), \href{https://docs.djangoproject.com/fr/2.2/topics/db/examples/many_to_one/}{voir des exemples}.

    \item[\mintinline{python3}{class OneToOneField(to, on_delete, parent_link=False, **options)}]~

          Une relation un-à-un, dont le fonctionnement est similaire à \mintinline{python3}{ForeignKey}; \href{https://docs.djangoproject.com/fr/2.2/topics/db/examples/one_to_one/}{voir des exemples}.

    \item[\mintinline{python3}{class ManyToManyField(to, **options)}]~

          Une relation plusieurs-à-plusieurs, qui fonctionne de la même manière que \mintinline{python3}{ForeignKey} (avec d'autres paramètres supplémentaires, cf. la doc); \href{https://docs.djangoproject.com/fr/2.2/topics/db/examples/many_to_many/}{voir des exemples}.
\end{description}

\subsubsection{Les options des champs}
Les champs acceptent des \mintinline{python3}{options}\marginpar{\href{https://docs.djangoproject.com/fr/2.2/ref/models/fields/\#field-options}{\mintinline{python3}{**options}}}, en voici quelques unes (on note après un signe \mintinline{python3}{=} la valeur par défaut):
\begin{description}
    ...
    \item[\mintinline{python3}{choices}]~
          ...
          C'est un itérable (tuple ou liste par exemple) constitué de couples \mintinline{python3}{(A, B)} où \mintinline{python3}{A} est la valeur réelle pour le modèle (valeur stockée en base) et \mintinline{python3}{B} le texte affiché à l'utilisateur. On peut organiser en sous groupe comme dans cet exemple:
...
\begin{description}
    ...
    \item[\mintinline{python3}{verbose_name}, \mintinline{python3}{verbose_name_plural}]~
          ...
          Nom de la table dans la base de données. Par défaut, Django la nomme \mintinline{python3}{application_modèle}.
        \end{description}
        
\subsubsection{Enregistrer des instances de modèles en base de données}
L'administration \marginpar{\mintinline{python3}{models}\par\quad\mintinline{python3}{.base}\par\quad\quad\href{https://docs.djangoproject.com/fr/2.2/ref/models/instances/}{\mintinline{python3}{.Model}}} permet de facilement modifier la base de données à la main, mais on doit utiliser l'API Django si on veut modifier la base de données à partir des vues ou des modèles eux-mêmes (par exemple, en reprenant l'exemple du didacticiel, on peut imaginer que la sauvegarde d'un objet \mintinline{python3}{Song} dans la base de données entraînera la création et sauvegarde de l'objet \mintinline{python3}{Artist} associé s'il n'existe pas).\medskip
...
\begin{description}
    \item[\mintinline{python3}{model_inst.clean_fields(exclude=None)}]~
    ...
    \item[\mintinline{python3}{model_inst.clean()}]~
    ...
    \item[\mintinline{python3}{model_inst.validate_unique(exclude=None)}]~

            Vérifie les contraintes d'unicité du modèle et lève une \mintinline{python3}{ValidationError} si échoue.

    \item[\mintinline{python3}{model_inst.full_clean(exclude=None, validate_unique=True)}]~

            Exécute les trois méthodes précédentes (exécute \mintinline{python3}{validate_unique} si le paramètre correspondant est \mintinline{python3}{True}).
\end{description}

Pour sauvegarder un objet en base, on appelle la méthode \mintinline{python3}{save()}:


\begin{description}
    \item[\mintinline{python3}{model_inst.save(force_insert=False, force_update=False, using=DEFAULT_DB_ALIAS, update_fields=None)}]~
    
    Sauvegarde l'objet en base. Le paramètre \mintinline{python3}{update_fields} permet de spécifier les champs à mettre à jour pour gagner en performance.
...
Le gestionnaire \marginpar{\href{https://docs.djangoproject.com/fr/2.2/topics/db/managers/}{\mintinline{python3}{models}}\par\quad\href{https://docs.djangoproject.com/fr/2.2/topics/db/managers/}{\mintinline{python3}{.manager}}\par\quad\quad\href{https://docs.djangoproject.com/fr/2.2/topics/db/managers/}{\mintinline{python3}{.Manager}}} est l'interface par laquelle on fait des requêtes à la base de données grâce à l'\href{https://docs.djangoproject.com/fr/2.2/ref/models/querysets/}{API des QuerySet} (voir le didacticiel pour un exemple, dans les vues \mintinline{python3}{artists_list} ou \mintinline{python3}{liste_chansons}). Le gestionnaire permet aussi bien d'inspecter la base de données que de la modifier.

\begin{description}
    \item[\mintinline{python3}{class Manager}]~
    
          Gestionnaire par défaut. Si aucun manager n'est défini explicitement dans un modèle, il sera automatiquement assigné à l'attribut de classe \mintinline{python3}{objects}.
        \end{description}
...
On peut définir une méthode spécifique pour cette requête dans un manager dérivé de \mintinline{python3}{models.Manager} :
...
\begin{description}
    \item[\mintinline{python3}{queryset.all()}]~

          Renvoie un \mintinline{python3}{QuerySet} contenant toutes les entrées de la table.

    \item[\mintinline{python3}{queryset.filter(**kwargs)}]~

          Renvoie un \mintinline{python3}{QuerySet} contenant tous les objets répondant aux \href{https://docs.djangoproject.com/fr/2.2/ref/models/querysets/#field-lookups}{paramètres rentrés}.

    \item[\mintinline{python3}{queryset.exclude(**kwargs)}]~

          Renvoie un \mintinline{python3}{QuerySet} contenant tous les objets sauf ceux répondant aux \href{https://docs.djangoproject.com/fr/2.2/ref/models/querysets/#field-lookups}{paramètres rentrés}.

    \item[\mintinline{python3}{queryset.reverse(**kwargs)}]~

          Renvoie le \mintinline{python3}{QuerySet} dans l'ordre inverse.

    \item[\mintinline{python3}{queryset.distinct(**kwargs)}]~
...
Il existe des méthodes de \mintinline{python3}{QuerySet} renvoyant autre chose qu'un \mintinline{python3}{QuerySet}. En voici quelques unes.

\begin{description}
    \item[\mintinline{python3}{queryset.get(**kwargs)}]~

        Renvoie \emph{l'unique} objet répondant aux paramètres rentrés. S'il existe plusieurs objets possibles, ou zéro objet possible, cette méthode lève une exception (respectivement \mintinline{python3}{MultipleObjectsReturned} et \mintinline{python3}{DoesNotExist}). Si une requête renvoie un \mintinline{python3}{QuerySet} singleton, on peut directement récupérer l'objet avec cette méthode sans paramètre (c'est risqué).

    \item[\mintinline{python3}{queryset.get_or_create(defaults=None, **kwargs)}]~

        Même comportement que ci-dessus, sauf que si l'objet n'existe pas, il est créé. Renvoie un tuple \mintinline{python3}{objet, créé} où \mintinline{python3}{objet} est l'objet créé ou charge, \mintinline{python3}{créé} un booléen: \mintinline{python3}{True} si l'objet a été créé et \mintinline{python3}{False} sinon. Cette méthode permet d'alléger la syntaxe et d'éviter d'effectuer diverses vérifications. Les méthodes permettant d'agir sur la base de données sont détaillées plus loin.

    \item[\mintinline{python3}{queryset.update_or_create(defaults=None, **kwargs)}]~
    ...
    \item[\mintinline{python3}{queryset.last()}]~
    ...
    \item[\mintinline{python3}{queryset.first()}]~

        Idem que la méthode précédente mais renvoie le premier objet.

    \item[\mintinline{python3}{queryset.latest(*fields)}]~
    ...
    \item[\mintinline{python3}{queryset.earliest(*fields)}]~
...
Comme \marginpar{\href{https://docs.djangoproject.com/fr/2.2/ref/request-response/}{\mintinline{python3}{django.http}}}indiqué dans le didacticiel:
\begin{enumerate}
...
On \marginpar{\href{https://docs.djangoproject.com/fr/2.2/ref/request-response/\#httpresponse-objects}{\mintinline{python3}{http.HttpResponse}}} s'intéresse maintenant à ce que les vues renvoient: les réponses HTTP.
...
\begin{description}
    \item[\mintinline{python3}{render(request, template_name, context=None, content_type=None, status=None, using=None)}]~
          ...
          Fonction qui combine un gabarit avec dictionnaire de contexte et renvoie une \mintinline{python3}{HttpResponse} avec le texte résultant. Deux paramètres obligatoires: \mintinline{python3}{request} et \mintinline{python3}{template_name}, le nom complet du gabarit à utiliser.
\end{description}
...
Les URL \marginpar{\href{https://docs.djangoproject.com/fr/2.2/ref/urls/}{\mintinline{python3}{django.urls}}} (Uniforme Resource Locators) sont gérés dans les différents fichiers \mintinline{python3}{urls.py}. Il y en a un dans chaque application et un dans le répertoire racine. Les URL sont configurées dans la liste \mintinline{python3}{urlpatterns}. Voici les principales fonctions à utiliser.
...
{% endfor %}
    \end{minted}
          A la première itération, \mintinline{python3}{"chaine_1"} sera utilisé, puis la chaîne contenu dans \mintinline{python3}{variable_de_la_chaine_2}, puis \mintinline{python3}{"chaine_3"}. Il est également possible de sauvegarder temporairement le paramètre dans une variable que l'on peut réutiliser plus loin.
...
\paragraph{Plus d'informations} \href{https://docs.djangoproject.com/fr/2.2/topics/forms/}{Documentation Django 2 --- Les formulaires}
...
Un formulaire doit hériter de la classe \marginpar{\href{https://docs.djangoproject.com/fr/2.2/ref/forms/api/\#django.forms.Form}{\mintinline{python3}{forms.Form}}}\mintinline{python3}{forms.Form}. De la même manière que pour
